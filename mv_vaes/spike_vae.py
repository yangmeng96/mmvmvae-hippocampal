...
class SPIKEVAE(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        latent_output = pd.DataFrame()
        for m in range(self.cfg.dataset.num_views):
            latent_output[str(m)+"x"] = enc_mu_val_orig[str(m)].cpu().numpy()[:, 0]
            latent_output[str(m)+"y"] = enc_mu_val_orig[str(m)].cpu().numpy()[:, 1]
        latent_output["aggx"] = enc_mu_val["0"].cpu().numpy()[:, 0]
        latent_output["aggy"] = enc_mu_val["0"].cpu().numpy()[:, 1]
        latent_output["odor"] = labels_val.cpu().numpy().astype(int)

        model_name = ''
        if self.cfg.model.name == 'mixedprior':
            if self.cfg.model.stdnormweight == 0.0:
                model_name = 'indep'
            else:
                model_name = 'ours'
        elif self.cfg.model.name == 'split':
            model_name = self.cfg.model.split_type
        elif self.cfg.model.name == 'joint':
            model_name = self.cfg.model.aggregation
        latent_output.to_csv(
            self.cfg.log.dir_logs+"/latent_output/"+model_name+"_latent_output.csv", 
            index=False)
        ...
        if self.cfg.eval.eval_downstream_task:
            ...
            self.log("val/downstream/acc", torch.tensor(mean_acc_views).mean())
            ...
            if self.cfg.model.name == "joint":
                ...
                # Save current final scores
                self.log("val/downstream/acc", torch.tensor(mean_acc_views_orig).mean())
        ...
        if self.cfg.eval.coherence:
            ...
            self.log("val/coherence/acc", torch.tensor(accs_coh).mean())
